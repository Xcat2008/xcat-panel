...
async function createVoiceOrAudioFiles(server, installConfig) {
  ...
  const startScriptLines = [
    ...
    '    ;;',
    '  teamspeak3)',
    '    runtime="/opt/gameforge/library/teamspeak3/server"',
...
async function createSinusBotFiles(server, installConfig) {
  ...
  await fs.writeFile(path.join(filesDir, 'start.sh'), [
    ...
    'fi',
    'SHARED_RUNTIME="/opt/gameforge/library/sinusbot/current"',
    'SHARED_TS3="/opt/gameforge/library/teamspeak3/client/TeamSpeak3-Client-linux_amd64"',
    'RUNTIME="$(pwd)/runtime"',
